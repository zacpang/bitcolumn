...
void read_insert_data()
{
	FILE *readdata = fopen("d:\\ÎÄµµ\\randomnumbers_0.1billion.txt", "r+");
	for (int i = 0; i < N; i++)
	{
...
int main(int argc, char ** argv) {
	...
	read_insert_data();
	tv2 = clock();
	printf("time_read:%lld\n", (tv2 - tv1));

	tv1 = clock();
	for (int i = 0; i < N; i++)
	{
		root = insert(root, data[i], data[i]);
	}
	tv2 = clock();
	printf("time_insert:%lld\n", (tv2 - tv1));

	record* res;
	tv1 = clock();
	int count = 0;
	for (int i = 1584408210; i < 1999999999; i += 10000)
	{
		res = find(root, i, 0);
		count++;
		if (res != NULL)
